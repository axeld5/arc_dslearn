"""Script for evaluating the RL model only."""
...
def evaluate_rl_model() -> Dict[str, float]:
    """Evaluate RL model with original DSL prompt."""
    # ──────────────────────────── bookkeeping dicts ─────────────────────────
    ...
    # ------------------------------------------------------------------ evaluation loop
    print("→ Evaluating RL model (with DSL prompt)")
    start = time()
    ...
    ax = top10.sort_values("error_rate")["error_rate"].plot(
        kind="barh",
        figsize=(7, 4),
        title="Top-10 DSL primitives by error-rate (RL - DSL prompt)",
    )
    ax.set_xlabel("error rate")
    ax.set_ylabel("DSL primitive")
    plt.tight_layout()
    plt.savefig("chart_results/error_rate_rl_only.png")
    plt.close()

    # ───────────────────── plot ❷ overall accuracy bar ───────────────────────
    acc_df = pd.Series(results).sort_values().to_frame("accuracy")
    ax = acc_df["accuracy"].plot(
        kind="barh", figsize=(6, 3), title="RL Model Accuracy on eval split (DSL prompt)"
    )
    ax.set_xlabel("accuracy")
    ax.set_xlim(0, 1)
    plt.tight_layout()
    plt.savefig("chart_results/rl_model_accuracy.png")
    plt.close()

    print("\nRL Model Functional accuracy on eval split (DSL prompt):")
    print(f"RL: {accuracy * 100:5.1f} %")
    print(f"(processed {len(raw_eval)} tasks in {runtime / 60:.1f} min)")
    print("\nSaved plots:")
    print(" • chart_results/rl_model_accuracy.png")
    print(" • chart_results/error_rate_rl_only.png")
