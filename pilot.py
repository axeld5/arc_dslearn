"""
make_prompts.py – generate few-shot input / output prompts
for every function in michaelhodel/arc-dsl.

Usage
-----
python make_prompts.py > arc_dsl_prompts.json
"""

import inspect
import json
import sys
import random
from pprint import pformat
from typing import Any, Callable, List, Sequence, get_origin, get_args


import arc_dsl.dsl as dsl  # ⇐ your cloned repo must be importable
import arc_dsl.arc_types as T  # provides Grid, Object, …

# ──────────────────────────────────────────────────────────────────────────
# 1. tiny helpers to build random ARC-ish values
#    (heuristics only – tune or replace with Hypothesis strategies)
# ──────────────────────────────────────────────────────────────────────────
def rand_color() -> int:
    return random.randint(0, 9)


def rand_grid(max_side: int = 5) -> T.Grid:
    h, w = random.randint(1, max_side), random.randint(1, max_side)
    return tuple(tuple(rand_color() for _ in range(w)) for _ in range(h))


def rand_object(max_side: int = 5) -> T.Object:
    g = rand_grid(max_side)
    triples = {
        (pix, (r, c))
        for r, row in enumerate(g)
        for c, pix in enumerate(row)
        if random.random() < 0.35
    }
    return frozenset(triples)

def rand_indices(max_side: int = 5) -> T.Indices:
    h, w = random.randint(1, max_side), random.randint(1, max_side)
    # pick 1-N random coordinates
    coords = {(random.randint(0, h - 1), random.randint(0, w - 1))
              for _ in range(random.randint(1, h * w))}
    return frozenset(coords)


def random_unary_dsl() -> Callable:
    """Return a random DSL function that takes exactly one positional arg."""
    unary = [
        f for _, f in inspect.getmembers(dsl, inspect.isfunction)
        if len(inspect.signature(f).parameters) == 1 and not f.__name__.startswith('_')
    ]
    return random.choice(unary) if unary else (lambda x: x)


def variant_generators(name: str, anno: Any) -> List[Callable[[], Any]]:
    """
    Return *all* distinct generators that make sense for this annotation.
    Each element is a zero-arg callable producing one admissible value.
    """
    # --- type-/alias-based ----------------------------------------------------
    if anno in {T.Patch, 'Patch'}:
        return [rand_object, rand_indices]

    if anno in {T.Object, 'Object'}:
        return [rand_object]

    if anno in {T.Indices, 'Indices'}:
        return [rand_indices]

    if anno in {T.Grid, 'Grid'} or 'grid' in name.lower():
        return [
            lambda: rand_grid(max_side=2),   # tiny, square/rect
            lambda: rand_grid(),             # arbitrary
        ]

    if anno in {bool, 'bool', T.Boolean}:
        return [lambda: True, lambda: False]

    if anno is Callable or 'Callable' in str(anno):
        return [
            lambda: (lambda x: x),           # identity lambda
            random_unary_dsl,                # real DSL primitive
        ]

    if anno is int or 'int' in str(anno):
        return [lambda: 0, lambda: random.randint(1, 4)]

    # --- name-based fall-back (same rules you had before) --------------------
    lname = name.lower()
    if any(k in lname for k in ['obj', 'patch', 'piece']):
        return [rand_object]
    if 'mask' in lname or 'matrix' in lname:
        return [lambda: tuple(tuple(int(bool(rand_color())) for _ in row)
                              for row in rand_grid())]
    if any(k in lname for k in ['coord', 'delta', 'offset']):
        return [lambda: (random.randint(-2, 2), random.randint(-2, 2))]

    # absolute last resort
    return [rand_color]



# ──────────────────────────────────────────────────────────────────────────
# 2. build one few-shot block for a single DSL function
# ──────────────────────────────────────────────────────────────────────────
def make_block(func: Callable,
               min_shots: int = 2,
               max_shots: int = 5) -> dict:
    sig = inspect.signature(func)

    # -------- collect variant generators for every *required* parameter ------
    param_variants = {
        name: variant_generators(name, param.annotation)
        for name, param in sig.parameters.items()
        if param.default is inspect._empty
           and param.kind is not inspect.Parameter.VAR_POSITIONAL
    }

    # how many examples do we need to cover every variant at least once?
    needed = max((len(v) for v in param_variants.values()), default=1)
    n_shots = max(needed, random.randint(min_shots, max_shots))

    shots: List[dict] = []

    for i in range(n_shots):
        kwargs = {
            name: (variants[i] if i < len(variants) else random.choice(variants))()
            for name, variants in param_variants.items()
        }

        # ------------- try to run the primitive; regenerate once on failure --
        try:
            out = func(**kwargs)
        except Exception:
            # one retry with fresh, random choices
            kwargs = {name: random.choice(variants)()
                      for name, variants in param_variants.items()}
            out = func(**kwargs)

        shots.append({"inputs": kwargs, "output": out})

    # ------------------------------ (prompt formatting identical) -----------
    demo_lines: List[str] = []
    for k, shot in enumerate(shots, start=1):
        demo_lines.append(f"# Example {k}")
        for arg_name, value in shot["inputs"].items():
            demo_lines.append(f"{arg_name.upper()} = {pformat(value)}")
        demo_lines.append(f"# Desired → {pformat(shot['output'])}")
        demo_lines.append("")

    input_prompt = (
        f"You are given {len(shots)} input/output examples. "
        f"Write solve(I) so that it applies {func.__name__} to its input.\n\n"
        + "\n".join(demo_lines)
        + f"### Now write solve(I) below"
    )

    output_prompt = (
        "def solve(I):\n"
        f"    return {func.__name__}(I)"
    )

    return {
        "name": func.__name__,
        "shots": shots,
        "input_prompt": input_prompt,
        "output_prompt": output_prompt,
    }



# ──────────────────────────────────────────────────────────────────────────
# 3. iterate over dsl.py and dump everything as JSON
# ──────────────────────────────────────────────────────────────────────────
def main() -> Sequence[dict]:
    blocks = []
    for name, func in inspect.getmembers(dsl, inspect.isfunction):
        if name.startswith("_"):  # skip private / helper funcs
            continue
        try:
            blocks.append(make_block(func))
        except Exception as err:
            print(f"[warn] skipped {name}: {err}", flush=True)
    return blocks


if __name__ == "__main__":
    with open("train_set.json", "w") as f:
        json.dump(main(), fp=f, indent=2, default=lambda o: pformat(o))
